...
def bst_cr_ind(D, R, fsw):
    ...
    """
    return np.format_float_scientific(np.divide(np.multiply(float(D)*np.power(1-float(D), 2), float(R)), np.multiply(2, float(fsw))), unique=False, precision=2, trim='-', exp_digits=1)
