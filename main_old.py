...
def save_to_history(specs: str, gvg_tf, gvd_tf, line_plot, ctrl_plot) -> None:
    # Use session state to persist history
    st.session_state["history_df"] = pd.concat([
        st.session_state["history_df"],
        pd.DataFrame({
            "Specs": [specs],
            "line_to_op_tf": [gvg_tf],
            "line_to_op_resp": [line_plot],
            "ctrl_to_op_tf": [gvd_tf],
...
def download_history_as_pdf_with_plots():
    ...
    with st.spinner("Generating PDF, please wait..."):
        ...
        for idx, row in st.session_state["history_df"].iterrows():
            ...
            for tf, plot in zip(
                [row["line_to_op_tf"], row["ctrl_to_op_tf"]],
                # ["Line to Output TF", "Line to Output Response", "Control to Output TF", "Control to Output Response"],
                ...
                with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as imgfile:
                    ...
                    pdf.cell(0, 6, tf, ln=True, align="C")
...
if cal_btn:
    try:
        ...
        with st.spinner("Calculating specs and plotting responses, please wait..."):
            ...
            st.write(op_string)
            st.write("#### Transfer Functions")
            st.latex(r"Line\ to\ Output\ Transfer\ Function,\ " + gvg_latex)
            st.plotly_chart(line_to_op_resp)
            st.latex(r"Control\ to\ Output\ Transfer\ Function,\ " + gvd_latex)
        ...
        # Save to history
        save_to_history(op_string, gvg_latex, gvd_latex, line_to_op_resp, ctrl_to_op_resp)
    ...
    if st.session_state["history_df"].empty:
        ...
        try:
            for index, row in st.session_state["history_df"].iterrows():
                ...
                with col_specs:
                    st.markdown(f"### Calculation {index + 1}")
                    st.markdown(row["Specs"] + f"{tf_to_latex(row["line_to_op_tf"], name='G_{vg}')}" + f"{tf_to_latex(row["ctrl_to_op_tf"], name='G_{vd}')}")
                with col_line:
                        st.plotly_chart(row["line_to_op_resp"])
                with col_ctrl:
                        st.plotly_chart(row["ctrl_to_op_resp"])
