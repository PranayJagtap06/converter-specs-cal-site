...
def bck_ind_current(D, Io):
    ...
    """
    return np.round(np.multiply(float(D), float(Io)), decimals=2)
...
def bck_cr_ind(D, R, fsw):
    ...
    """
    return np.format_float_scientific(np.divide(np.multiply(1-float(D), float(R)), np.multiply(2, float(fsw))), unique=False, precision=2, trim='-', exp_digits=1)
