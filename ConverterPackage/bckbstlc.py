...
def bckbst_duty_cycle(Vo, Vin):
    ...
    Value of duty cycle.
    """
    return np.round(np.divide(float(Vo), np.add(float(Vo), float(Vin))), decimals=3)


def bckbst_ind_current(D, Io):
    """
...
def bckbst_ripl_current(IL, Irp):
    """
...
def bckbst_cr_ind(D, R, fsw):
    ...
    """
    return np.format_float_scientific(np.divide(np.multiply(np.power(1-float(D), 2), float(R)), np.multiply(float(fsw), 2)), unique=False, precision=2, trim='-', exp_digits=1)


def bckbst_cont_ind(Vin, D, fsw, I_ripple):
    """
...
def bckbst_ind_ripl_(Vin, D, fsw, L):
    """
...
def bckbst_cap_val(D, R, Vrp, fsw):
    """
    ...
    """
    return np.format_float_scientific(float(D) / ((float(R)*float(fsw))*float(Vrp/100)), unique=False, precision=2, trim='-', exp_digits=1)

def Esr(Vrp, Vo, max_indcrnt):
    """
    ...
    Vrp : % Ripple Voltage.
    Vo : Output Voltage.
    max_indcrnt : Max Inductor current.

    Returns
    -------
    Value of ESR.
    """
    return np.round(np.divide(np.multiply(float(Vrp/100), float(Vo)), max_indcrnt), decimals=3)
